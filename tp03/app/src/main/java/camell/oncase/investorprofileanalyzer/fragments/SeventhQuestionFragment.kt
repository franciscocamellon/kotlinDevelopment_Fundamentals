...
import camell.oncase.investorprofileanalyzer.databinding.FragmentSeventhQuestionBinding
import camell.oncase.investorprofileanalyzer.viewmodel.InvestorProfileViewModel

class SeventhQuestionFragment : Fragment() {
    private var _binding: FragmentSeventhQuestionBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentSeventhQuestionBinding.inflate(inflater, container, false)
    ...
    private fun goToNextScreen(view: View) {
        ...
        binding.seventhQuestionNextButton.setOnClickListener{

            val selectedOption: Int = binding.seventhQuestionRadioGroup.checkedRadioButtonId

            if (selectedOption == -1) {
                Toast.makeText(context, getString(R.string.unselected_radio_button), Toast.LENGTH_SHORT).show()
            } else {
                Navigation.findNavController(view).navigate(R.id.action_seventhQuestionFragment_to_eighthQuestionFragment)
            }
        }
